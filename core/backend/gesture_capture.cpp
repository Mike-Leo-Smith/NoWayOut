...
GestureCapture::GestureCapture(std::string address) {
    ...
    _update_thread = std::thread{[ip_address = std::move(address), this] {
        ...
        for (;;) {
            ...
            _state.nodes[0] = {coords[1], coords[3], config::upper_arm_radius};  // left upper arm
            _state.nodes[1] = {coords[3], coords[4], config::lower_arm_radius};  // left lower arm
            _state.nodes[2] = {coords[1], coords[6], config::upper_arm_radius};  // right upper arm
            _state.nodes[3] = {coords[6], coords[7], config::lower_arm_radius};  // right lower arm
            
            _state.nodes[4] = {coords[8], coords[10], config::upper_leg_radius};  // left upper leg
            _state.nodes[5] = {coords[10], coords[11], config::lower_leg_radius};  // left lower leg
            _state.nodes[6] = {coords[8], coords[13], config::upper_leg_radius};  // right upper leg
            _state.nodes[7] = {coords[13], coords[14], config::lower_leg_radius};  // right lower leg
            ...
            _state.neck = {coords[0], coords[1], config::neck_radius};  // neck
            
            _state.nodes[11] = {coords[0], coords[0] + glm::vec3{config::head_radius * 2.0f, 0.0f, 0.0f}, config::head_radius};  // head
